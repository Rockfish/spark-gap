...
///
/// From Bevy crates/bevy_transform/src/components/transform.rs
///
...
impl Transform {
    ...
    /// added here, not in bevy
    #[inline]
    #[must_use]
    pub fn mul_transform_weighted(&self, transform: Transform, weight: f32) -> Self {
        let translation = self.translation.lerp(transform.translation, weight);
        let rotation = self.rotation.slerp(transform.rotation, weight);
        let scale = self.scale.lerp(transform.scale, weight);
        Transform {
            translation,
            rotation,
            scale,
